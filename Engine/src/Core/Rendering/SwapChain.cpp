...
SwapChain::SwapChain(std::shared_ptr<Device> deviceRef, VkExtent2D extent)
    : device(std::move(deviceRef)), windowExtent{extent} {
  createSwapChain();
  createImageViews();
  createRenderPass();
...
VkExtent2D SwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
  if (capabilities.currentExtent.width != UINT32_MAX) {
    ...
    actualExtent.width = NK_MAX(
        capabilities.minImageExtent.width,
        NK_MIN(capabilities.maxImageExtent.width, actualExtent.width));
    actualExtent.height = NK_MAX(
        capabilities.minImageExtent.height,
        NK_MIN(capabilities.maxImageExtent.height, actualExtent.height));
